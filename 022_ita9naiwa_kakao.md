...
C-WARP의 핵심 학습 구조와 모듈 통합성은 우수하지만, 실행 불가능한 accelerator 분기·반환 계약 불일치·샘플링 엣지 케이스 미방어로 인해 운영 환경에서는 예측 불가능한 장애를 유발할 수 있는 연구용 안정성 수준에 머문다.
...
class WARP(Algo, WARPOption, Evaluable, Serializable):
    """Python implementation for C-WARP.
    """
    def __init__(self, opt_path=None, seed: Optional[int] = None, *args, **kwargs):
        Algo.__init__(self, *args, **kwargs)
        WARPOption.__init__(self, *args, **kwargs)
        Evaluable.__init__(self, *args, **kwargs)
        Serializable.__init__(self, *args, **kwargs)
        ...
        # [방어적 예외 처리] assert 제거 후 명시적 예외 처리로 운영 무결성 강화
        if not self.obj.init(bytes(self.opt_path, "utf-8")):
            raise RuntimeError(f"Cannot parse option file: {opt_path}")
        ...
        if self.data:
            self.logger.info(self.data.show_info())
            if self.data.data_type not in ["matrix"]:
                raise ValueError(f"Unsupported data type: {self.data.data_type}. Expected 'matrix'.")
                
        if isinstance(self.opt.score_func, str):
            self.opt.score_func = self.opt.score_func.lower()

        # [재현성(Reproducibility) 확보] 독립된 RandomState 객체 주입 관리
        model_seed = seed if seed is not None else getattr(self.opt, "seed", 42)
        self.rng = np.random.RandomState(model_seed)

    @staticmethod
    def new(path, data_fields=None):
        # [안티패턴 수정] mutable default argument(data_fields=[]) 제거
        if data_fields is None:
            data_fields = []
        return WARP.instantiate(WARPOption, path, data_fields)

    def set_data(self, data):
        if not isinstance(data, aux.data.Data):
            raise TypeError(f"Wrong instance type: {type(data)}")
        self.data = data
    ...
    def initialize(self):
        super().initialize()
        # [방어적 예외 처리] assert 제거 후 명시적 에러 발생
        if self.data is None:
            raise ValueError("Data is not set. Please call set_data() or pass data configuration.")
        self.buf = BufferedDataMatrix()
        self.buf.initialize(self.data)
        self.init_factors()
    ...
    def init_factors(self):
        ...
        # [재현성 확보] 전역 난수 대신 self.rng 활용
        self.P = self.rng.normal(scale=1.0 / (self.opt.d ** 2),
                                 size=(header["num_users"], self.opt.d)).astype("float32")
        self.Q = self.rng.normal(scale=1.0 / (self.opt.d ** 2),
                                 size=(header["num_items"], self.opt.d)).astype("float32")
        self.Qb = self.rng.normal(scale=1.0 / (self.opt.d ** 2),
                                  size=(header["num_items"], 1)).astype("float32")
        if not self.opt.use_bias:
            self.Qb *= 0
        self.obj.initialize_model(self.P, self.Q, self.Qb, self.num_nnz)
    ...
    def sampling_loss_samples(self):
        users, positives, negatives = [], [], []
        skipped_users = 0  # [관측성 추가] 샘플링 스킵 유저 카운터 도입
        
        if self.opt.compute_loss_on_training:
            self.logger.info("Sampling loss samples...")
            header = self.data.get_header()
            num_loss_samples = int(header["num_users"] ** 0.5)
            _users = self.rng.choice(range(self.P.shape[0]), size=num_loss_samples, replace=False)
            
            num_items = self.Q.shape[0]
            for u in _users:
                keys, *_ = self.data.get(u)
                if len(keys) == 0:
                    skipped_users += 1
                ...
                sample_size = min(len(seen) + 1, num_items)
                negs = self.rng.choice(range(num_items),
                                       size=sample_size,
                                       replace=False)
                negs = [n for n in negs if n not in seen]
                if not negs:
            ...
            if skipped_users > 0:
                self.logger.warning(f"Skipped {skipped_users} users during negative sampling due to constraints or empty history.")
            self.logger.info("Generated %s loss samples." % len(users))
    ...
    def _finalize_train(self) -> float:
        result = self.obj.join()
...
최종개선사항
✅ assert 기반 런타임 검증 제거 → 명시적 RuntimeError/ValueError/TypeError 예외 처리 전환
✅ 전역 numpy.random 사용 제거 → self.rng 기반 난수 관리로 학습 재현성 확보
✅ data_fields=[] mutable default 제거 → None 초기화 패턴으로 참조 오염 방지
✅ GPU 미구현 분기 제거 → C-Worker 전용 학습 흐름으로 Dead Code 최소화
✅ _finalize_train() 반환 계약 통일 → float 타입 보장으로 인터페이스 안정성 확보
✅ Negative Sampling 예외 케이스 추가 방어 → 전체 아이템 소비 유저 및 빈 히스토리 유저 안전 제외
✅ Sampling 실패 사용자 카운터 추가 → 데이터 품질 문제 추적 가능성 확보
✅ 샘플링 난수 호출 변경 → 전역 상태 의존 제거 및 실험 결과 일관성 강화
✅ C++ Worker 종료 결과 검증 추가 → None 반환 상황에서도 엔진 셧다운 방지
✅ 초기화 단계 검증 강화 → 잘못된 옵션/데이터 입력 시 조용한 실패 방지

현재 수준은 단순 패치가 아니라 학습 엔진 운영 환경 기준의 방어적 리팩토링 완료 상태에 가깝습니다. 남은 개선점은 대규모 분산 학습 환경에서의 체크포인트 원자성, Worker 장애 복구, 메트릭 수집 정도입니다.
