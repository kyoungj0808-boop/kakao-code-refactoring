...
상급 LLM Fine-tuning 파이프라인 구조를 갖췄지만, ZeRO-3 저장 동기화·하드코딩 분기·운영 검증 계층 부족으로 대규모 분산 학습 환경에서는 안정성을 잃을 수 있는 연구용 코드다.
...
# [결정적 import 확보] sys.path.insert(0, ...)를 사용하여 환경별 우선순위 오염 및 충돌 방지
sys.path.insert(
    0,
...
def get_local_rank() -> int:
    """[분산 안정성] 모듈 임포트 시점이 아닌 런타임에 동적으로 LOCAL_RANK를 평가하여 상태 오염 방지"""
...
class DataArguments:
    ...
    eval_ratio: float = field(
        default=1.0, metadata={"help": "percentage of data used for evaluation"}
    )
    packing: bool = field(
        default=False, metadata={"help": "Whether use packing or not"}
    )
...
def find_all_linear_names(model):
    ...
    if "lm_head" in lora_module_names:
        lora_module_names.remove("lm_head")
    # [재현성 확보] set의 비결정적 순서로 인한 LoRA 체크포인트 깨짐 방지를 위해 sorted list 반환
...
def print_trainable_parameters(model):
    ...
    trainable_params = embedding_lm_head_param_count + lora_param_count
    print_rank0(
        f"all params: {all_param:,d} || trainable params: {trainable_params:,d} || trainable%: {100 * trainable_params / all_param if all_param > 0 else 0}"
    )
...
def get_device_map(
    ...
    if lora_args.q_lora:
        if ddp and training_args.fsdp:
            print("FSDP is incompatible with QLORA")
        device_map = {"": get_local_rank()} if ddp else None
        if len(training_args.fsdp) > 0 or deepspeed.is_deepspeed_zero3_enabled():
            print("FSDP and ZeRO3 are both currently incompatible with QLoRA.")
    return device_map
...
def prepare_model_for_training(
    ...
    if getattr(model.config, "output_router_logits", False) or hasattr(model.config, "router_aux_loss_coef"):
        setattr(model.config, "output_router_logits", True)
        print_rank0("Activate computing load balancing loss for MoE architecture")
...
def get_peft_state_maybe_zero_3(named_params, bias):
    if bias == "none":
        # [무결성 강화] 접두사 패턴(base_model.model.)과 lora_ 키를 엄격하게 매칭하여 외부 커스텀 모듈 오탐 및 누락 방지
        to_return = {
            k: t for k, t in named_params 
            if k.startswith("base_model.model.") and "lora_" in k
        }
    elif bias == "all":
        to_return = {
            k: t for k, t in named_params 
            if (k.startswith("base_model.model.") and "lora_" in k) or "bias" in k
        }
    elif bias == "lora_only":
        to_return = {}
        maybe_lora_bias = {}
...
def print_some_examples(ds, tokenizer):
    ...
    for batch in data_loader:
        ...
        print_rank0("--------------****Example data point****---------------")
        print_rank0("device: ", batch["input_ids"].device)
        print_rank0("shape of input_ids: ", batch["input_ids"].shape)
        print_rank0("shape of labels: ", batch["labels"].shape)
        print_rank0("shape of attention_mask: ", batch["attention_mask"].shape)
        print_rank0("attention mask: ", batch["attention_mask"])
        input_ids = batch["input_ids"][0].tolist()
        ...
        # [명시적 예외 처리] python -O 옵션에서도 안전하게 동작하도록 assert 제거 및 명시적 ValueError 전환
        if len(input_chunk) != 1:
            raise ValueError(
                f"Invalid packed input structure: expected 1 unmasked chunk, but got {len(input_chunk)}"
            )
...
def train():
    ...
    if data_args.train_data_path is None:
        raise ValueError("Please provide a training data file.")
    ...
    def compute_metrics(eval_preds):
        ...
        return {"accuracy": acc_count / total_num if total_num > 0 else 0.0, "perplexity": perplexity}

    is_moe_model = getattr(model.config, "output_router_logits", False) or hasattr(model.config, "router_aux_loss_coef")
    ...
    if training_args.do_eval:
        trainer = Trainer(
            ...
            train_dataset=train_dataset,
            eval_dataset=eval_dataset,
            compute_metrics=None if is_moe_model else compute_metrics,
            preprocess_logits_for_metrics=None if is_moe_model else preprocess_logits_for_metrics,
        )
    else:
        trainer = Trainer(
            model=model,
    ...
    # [분산 장애 방지] ZeRO-3 집단 통신(Collective Communication) 진입 전후 barrier 동기화 추가로 NCCL 락업(Deadlock) 원천 차단
    state_dict = None
    if deepspeed.is_deepspeed_zero3_enabled():
...
최종개선사항
✅ ZeRO-3 저장 구간 barrier 추가 → NCCL Collective Deadlock 방지 구조 확보
✅ set 기반 LoRA target module 정렬 → 체크포인트 재현성 강화
✅ sys.path.append 제거 → 결정적 import 우선순위 확보
✅ assert 검증 제거 → 운영 환경 예외 감지 보장
✅ MoE 모델 감지 문자열 의존 제거 → config 기반 확장 구조 전환
✅ LoRA state_dict 필터 강화 → 커스텀 모듈 오탐 방지
✅ train 데이터 검증 방식 개선 → 런타임 장애 조기 탐지
✅ perplexity 계산 방어 추가 → overflow 및 zero division 방지

현재 수준은 연구용 스크립트를 넘어 대규모 LLM 파인튜닝 운영 파이프라인에 투입 가능한 단계이며, 남은 개선 영역은 checkpoint atomic save, SIGTERM 복구, experiment metadata tracking 같은 MLOps 영역이다.
